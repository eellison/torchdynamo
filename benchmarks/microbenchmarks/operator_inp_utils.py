...
def map_json_to_torch_args(arg):
    if isinstance(arg, list):
        ...
        if "torch.Tensor" in arg:
            ...
            kwargs = {field: map_json_to_torch_args(fields_dict[field]) for field in fields}
            return torch.empty_strided(**kwargs)
        for key in ("torch.dtype", "torch.device", "torch.layout"):
            if key in arg:
                return  eval(arg[key])
        return {map_json_to_torch_args(k): map_json_to_torch_args(v) for k,v in arg.items()}
    else:
        return arg
...
class OperatorInputsMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func_overload, types, args=(), kwargs=None):
        ...
        inps = ((args, kwargs))
        # TODO: need to serialize embedding bag inputs, because the computation
        # depends on the data
        # or make representative inputs
...
class OperatorInputsLoader:
    ...
    def get_inputs_for_operator(self, operator, dtype, device="cuda") -> Generator[Tuple[Iterable[Any], Dict[str, Any]], None, None]:
        assert str(operator) in self.loaded_json_dict, f"Could not find {operator}"
